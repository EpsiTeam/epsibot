...
\`${pre}command remove test\` => retire la commande \`${pre}test\``
	...
	async execute({msg, args, prefix}, {db, log, serverCommand}) {
		if (!msg.guild) return // Shouldn't happen
		...
		if (!cmdToRemove) {
			return msg.channel.send(epsimpleembed(
				"il faut me donner la commande à retirer pour que je puisse le faire",
				id,
				"RED"
			))
		}
		...
		if (!cmdExist) {
			return msg.channel.send(epsimpleembed(
				`la commande personnalisée \`${prefix}${cmdToRemove}\` n'existe pas`,
				id,
				"RED"
			))
		}
		...
		return msg.channel.send(epsimpleembed(
			`la commande \`${prefix}${cmdToRemove}\` a été supprimée`,
			id,
			"GREEN"
		))
	}
